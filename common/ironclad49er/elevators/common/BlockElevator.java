...
public class BlockElevator extends BlockContainer{
    ...
    private static void say(String s) { say(s, false);}
    private static void say(String s, boolean always) {if (always || verbose) { mod_Elevator.say(s, always);}} 
    ...
    private static void openGUI(World world, ChunkPosition loc, EntityPlayer player) {	
        mod_Elevator.packetHandler.requestGUIMapping(world, loc, player);
    }   
    ...
    public int getBlockTextureFromSideAndMetadata(int side, int meta) {
    	if (isCeiling(meta)) {
    		if (side != 0) {return mod_Elevator.sideTexture;}
    		else {return mod_Elevator.topTexture;}
    	}
    	else {
	    	if (side != 1) {return mod_Elevator.sideTexture;}
	        else {return mod_Elevator.topTexture;}
    	}
    }

    public static TileEntityElevator getTileEntity(World world, int i, int j, int k) {
    	if (isCeiling(world, i, j, k)) {return null;}
    	if (world.getBlockId(i, j, k) != mod_Elevator.Elevator.blockID) {
    		say("That's not an elevator.");
    		return null;
    	}
    	TileEntityElevator tile = (TileEntityElevator)world.getBlockTileEntity(i, j, k);
    	if (tile != null && tile instanceof TileEntityElevator) {
    		return tile;
    	}
    	else {
    		world.setBlockTileEntity(i, j, k, new TileEntityElevator());
    		TileEntityElevator tileTry2 = (TileEntityElevator)world.getBlockTileEntity(i, j, k);
    ...
    public static TileEntityElevator getTileEntity(IBlockAccess world, int i, int j, int k) {
    	if (isCeiling(world.getBlockMetadata(i, j, k))) { return null; }
    	if (world.getBlockId(i, j, k) != mod_Elevator.Elevator.blockID) {
    		say("That's not an elevator.");
    		return null;
    	}
    ...
    public void onBlockAdded(World world, int i, int j, int k) {
        ...
        mod_Elevator.elevator_powerOn(world, curPos);
        say("Elevator Added: " + mod_Elevator.pos2Str(curPos));
    }
    ...
    public void breakBlock(World world, int i, int j, int k, int blockID, int metadata) {
		...
		say("Elevator Removed: " + mod_Elevator.pos2Str(curPos));
	...
	public static boolean hasCeiling(World world, ChunkPosition pos) { return hasCeiling(world, pos.x, pos.y, pos.z); }
	public static boolean hasCeiling(World world, int i, int j, int k) {
		if (j > mod_Elevator.max_elevator_Y - 3) {return false;}
		if (isCeiling(world, i, j, k)) {return false;}
		return (world.getBlockId(i, j + 3, k) == mod_Elevator.Elevator.blockID);
	}
	...
	public static boolean isCeiling(World world, ChunkPosition pos) { return isCeiling(world, pos.x, pos.y, pos.z); }
	public static boolean isCeiling(World world, int i, int j, int k) { return isCeiling(world.getBlockMetadata(i, j, k)); }
	public static boolean isCeiling(int meta) { return ((meta & 0x01) == 1); }
   
    public void updateCeilingStatus(World world, int i, int j, int k) {
    	if (world.getBlockId(i, j, k) != mod_Elevator.Elevator.blockID) {return;}
    ...
    public static boolean hasOpening(World world, ChunkPosition pos, int origY, boolean strict, boolean testingShaft) {
    	return hasOpening(world, pos.x, pos.y, pos.z, origY, strict, testingShaft);
    }
    public static boolean hasOpening(World world, int x, int y, int z, int origY, boolean strict, boolean testingShaft) {
    	...
    	if (y != origY && !world.isAirBlock(x, y, z) && !(ceil && y == origY + 3) && !testingShaft) {return false;}
    	if (!strict && !ceil) {return true;}
    	
    	for (int testY = y + 1; testY <= max; testY++) {
    		boolean valid = mod_Elevator.isBlockOpeningMaterial(world, x, testY, z) || origY == testY;
    ...
    public static boolean hasPossibleFloor(World world, int x, int y, int z, int origY) {
    	...
    	if (!hasOpening(world, x, y, z, origY, true, false)) {return false;}
    	if (hasCeiling(world, x, origY, y) && (y + 3) >= mod_Elevator.max_elevator_Y) { return false; }
  
    	//If all of those didn't already kick us out of the function, check for ledges
    	for (int iter = 0; iter < 4; iter++ ) {
    		int tempX = x; int tempZ = z;
    		if (iter == 0) {tempZ--;} else if (iter == 1) {tempZ++;}
    		else if (iter == 2) {tempX--;} else if (iter == 3) {tempX++;}
    		if (mod_Elevator.isBlockLedgeMaterial(world, tempX, y, tempZ) && mod_Elevator.isBlockOpeningMaterial(world, tempX, y + 1, tempZ) && mod_Elevator.isBlockOpeningMaterial(world, tempX, y + 2, tempZ)) {
    			return true;
    		}
    	}
    	return false;
    ...
    public static Set<Integer> refreshElevator(World world, ChunkPosition pos) {return refreshElevator(world, pos.x, pos.y, pos.z);}
    public static Set<Integer> refreshElevator(World world, int x, int y, int z) {

    	Set<Integer> blockNum = new HashSet<Integer>();
    	if (world.getBlockId(x, y, z) != mod_Elevator.Elevator.blockID) {return blockNum;}
    	
    	int curFloor = 0;
    	boolean betweenFloors = true;
    	
    	for(int curY = y; curY > 0 && (curY == y || mod_Elevator.isBlockOpeningMaterial(world, x, curY, z) ); curY-- ) {
    		if (hasPossibleFloor(world, x, curY, z, y)) {
    			blockNum.add(curY);
    			if (curY == y) {betweenFloors = false;}
    		}
    	}
    	
    	boolean addCheck = true;
    	for(int curY = y + 1; curY< (mod_Elevator.max_elevator_Y - 3) && (mod_Elevator.isBlockOpeningMaterial(world, x, curY, z) || (curY == y + 3 && hasCeiling(world, x, y, z))); curY++ ) {
    		if (hasPossibleFloor(world, x, curY, z, y) ) {
    			blockNum.add(curY);
    			addCheck = false;
    		}
    ...
    public static boolean isReachable(World world, ChunkPosition testFloor, int origY) {
    	return isReachable(world, testFloor, origY, mod_Elevator.strictShaft);
    }
    public static boolean isReachable(World world, ChunkPosition testFloor, int origY, boolean strict) {
    	if (testFloor.y <= 0 || testFloor.y >= mod_Elevator.max_elevator_Y) { return false; }
    	if (hasCeiling(world, testFloor.x, origY, testFloor.z) && (testFloor.y + 3) >= mod_Elevator.max_elevator_Y) { return false; }
    	
    	if (origY < testFloor.y) {
    		for (int testY = origY; testY <= testFloor.y; testY++) {
	    		if(!hasOpening(world, testFloor.x, testY, testFloor.z, origY, strict, (testY != origY && testY != testFloor.y))) {
	    			say("Blocked at: " + testFloor.x + ", " + testY + ", " + testFloor.z);
	    			return false;
	    		}
	    	}
    	}
    	else if (origY == testFloor.y) {return true;}
    	else {
    		for (int testY = origY; testY >= testFloor.y; testY--) {
    			if(!hasOpening(world, testFloor.x, testY, testFloor.z, origY, strict, (testY != origY && testY != testFloor.y))) {
    				say("Blocked at: " + testFloor.x + ", " + testY + ", " + testFloor.z);
    ...
    public static void refreshAndCombineAllAdjacentElevators(World world, ChunkPosition pos, boolean justAdded) {
    	...
    	if (!justAdded && thisTile != null) {
    		try {
    			updateProperties = checkoutWaitingNames(world, pos.x, pos.y, pos.z);
    	    	if (updateProperties) {
    	...
    	while(iter.hasNext()) {
        	...
        	if (updateProperties) {
	        	TileEntityElevator curInfo = getTileEntity(world, curPos.x, curPos.y, curPos.z);
	        	try {
    	...
    	while (it.hasNext()) {
    		...
    		if (verbose) {say((new StringBuilder()).append("testing y = ").append(curTestFloorY).toString());}
    		boolean canBeUsed = true;
    		for(int i = 0; i < conjoinedElevators.size() && canBeUsed; i++) {
    			ChunkPosition curElvPos = conjoinedElevators.get(i);
    			if(!isReachable(world, new ChunkPosition(curElvPos.x, curTestFloorY, curElvPos.z), curElvPos.y)) {canBeUsed = false;}
    		}
    		if (!canBeUsed) {invalids.add(curTestFloorY);}
    	}
    	if (verbose) {
    		say("----------------------------------------------------");
    ...
    public static void listFloors(Set<Integer> floors) {
		...
		while (i.hasNext()) {
			say( (new StringBuilder()).append("Floor at y = ").append(i.next()).append(".").toString());
		}
    }
    ...
    public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int side, float par7, float par8, float par9) {
        ...
        if (curItem != null && curItem.itemID == mod_Elevator.Elevator.blockID) {
        	return false;
        }
        ...
        if (elevatorInfo == null) {
        	player.addChatMessage(mod_Elevator.message_elevator_outoforder);
        	return true;
        }
		...
		say("Player is on elevator: " + isEntityOnThisElevator(world, loc, player));
		
		/*if (mod_Elevator.canShortCircuit(player.username) && floorsBelow == 1 && ( (!isCeiling && hasCeiling(world, loc)) || floorsAbove == 0) && isEntityOnThisElevator(world, loc, player)) {
			say("'Short circuit' request to bottom floor!");
			mod_Elevator.elevator_requestFloor(world, loc, curFloor - 1);
		}
		else if (mod_Elevator.canShortCircuit(player.username) && floorsAbove == 1 && (isCeiling || floorsBelow == 0) && isEntityOnThisElevator(world, loc, player)) {
    ...
    public boolean isEntityOnThisElevator(World world, ChunkPosition pos, EntityPlayer player) {
    	Iterator<ChunkPosition> iter = conjoinedElevators.iterator();
    	while (iter.hasNext()) {
    		if(mod_Elevator.isEntityOnBlock(world, iter.next(), player) ) {return true;}
    	}
    	return false;
    }
    
    public static void resetAdjacenciesList(World world, ChunkPosition startPos) {	
    	if (!conjoinedElevators.isEmpty()) { conjoinedElevators.clear(); }
    ...
    public static void populateAdjacenciesList(World world, ChunkPosition pos, int dist) {
		...
		if (dist > 127) {return;}
    	for (int iter = 0; iter < 4; iter++) {
    		int curX = pos.x; int curZ = pos.z; 
			if (iter == 0) {curX ++;} else if (iter == 1) {curX --;}
			else if (iter == 2) {curZ ++;} else if (iter == 3) {curZ--;}
			
			ChunkPosition curPos = new ChunkPosition(curX, pos.y ,curZ);
			if (!conjoinedElevators.contains(curPos) && world.getBlockId(curX, pos.y, curZ) == mod_Elevator.Elevator.blockID) {
    ...
    public boolean isPoweringTo(IBlockAccess iblockaccess, int i, int j, int k, int l) {
    	if (isCeiling(iblockaccess.getBlockMetadata(i, j, k))) {return false;}
    	if (l == 0) {return false;}
    	else {
    		TileEntityElevator elevatorInfo = getTileEntity(iblockaccess, i, j, k);
    ...
    public void updateTick(World world, int i, int j, int k, Random random) {
    	...
    	say("Elevator Updated at " + mod_Elevator.pos2Str(curPos));
    	...
    	switch (curState) {
    		...
    		case DEMAND_NEW_FLOOR:
    			say("Block has been demanded to " + curDest);
    	   		for (int iter = 0; iter < conjoinedElevators.size() && (curDest != j); iter++) {
    	       		...
    	       		if (!isReachable(world, new ChunkPosition(conjPos.x, curDest, conjPos.z), conjPos.y, false)) {
    	       			curDest = j;
    	       			say("Unable to meet demand - part of the elevator is blocked!");
    	       			say("Elevator chunk blocked at: " + mod_Elevator.pos2Str(conjPos));
    	       		}
    	   		}
    	   		if (curDest == j) {
    	   			elevatorInfo.clearState();
    		...
    		case REQUEST_NEW_FLOOR:
    			if (providesPower && curDest != j) {
    				...
    				mod_Elevator.refreshElevator(world, curPos, 2);
    			}
    			else if (!world.isRemote && (curDest != j) && (elevatorInfo.hasFloorAt(curDest) || curState == DEMAND_NEW_FLOOR)) {
    	    		...
    	    		for (int iter = 0; iter < conjoinedElevators.size(); iter++) {
    					...
    					TileEntityElevator curInfo = getTileEntity(world, pos.x, pos.y, pos.z);
    			    	
    					if (curInfo != null && !isCeiling(world, pos)) {
    		    			curInfo.demandY(curDest);

    		    			int metadata = world.getBlockMetadata(pos.x, pos.y, pos.z);
    		    			boolean isCenter = (pos.x == i && pos.y == j && pos.z == k); //|| isClient;
    			    		EntityElevator curElevator = new EntityElevator(world, pos.x, pos.y, pos.z);
    		    			if (hasCeiling(world, pos)) {
    			    			// Create ceiling entity
    			    			EntityElevator ceilingElevator = new EntityElevator(world, pos.x, pos.y + 3, pos.z);
    			    			
    			    			ceilingElevator.setProperties(curDest + 3, false, isClient, world.getBlockMetadata(pos.x, pos.y + 3, pos.z));
    		    			...
    		    			curElevator.setProperties(curDest, isCenter, isClient, metadata);
