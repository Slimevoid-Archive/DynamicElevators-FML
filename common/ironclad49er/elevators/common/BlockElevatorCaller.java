...
public class BlockElevatorCaller extends Block {
	...
	private boolean isBeingPoweredByNonElevator(World world, int i, int j, int k) {
		for (int iter = 0; iter < 6; iter++ ) {
    		int tempX = i; int tempZ = k; int tempY = j;
    		if      (iter == 0) {tempY--;} else if (iter == 1) {tempY++;}
    		else if (iter == 2) {tempZ--;} else if (iter == 3) {tempZ++;}
    		else if (iter == 4) {tempX--;} else if (iter == 5) {tempX++;}
    		int ID = world.getBlockId(tempX, tempY, tempZ);
    		mod_Elevator.say("Checking: " + tempX + ", " + tempY + ", " + tempZ + ": has block ID" + ID);
    		if (ID > 0 && ID != mod_Elevator.Elevator.blockID && Block.blocksList[ID].isIndirectlyPoweringTo(world, tempX, tempY, tempZ, iter) ) {
    			return true;
    		}
		}
		return false;
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, int notifierID) {
        if (notifierID != mod_Elevator.ElevatorButton.blockID) {
			if (notifierID <= 0 || notifierID == mod_Elevator.Elevator.blockID || !Block.blocksList[notifierID].canProvidePower() || !isBeingPoweredByNonElevator(world, i, j, k)) {
	        	previouslyPowered = false;
	        	return;
	        }
			...
			for (int iter = 0; iter < 6; iter++ ) {
	    		int tempX = i; int tempZ = k; int tempY = j;
	    		if      (iter == 0) {tempX--;} else if (iter == 1) {tempX++;}
	    		else if (iter == 2) {tempZ--;} else if (iter == 3) {tempZ++;}
	    		if (world.getBlockId(tempX,  j, tempZ) == mod_Elevator.ElevatorButton.blockID &&
	...
	public static boolean findAndActivateElevator(World world, int i, int j, int k, int depth) {
		...
		// Look for adjacent elevator caller blocks - if any are found, activate them.
		// Also, if any are found, do not search at this location
    	boolean foundOtherCallerBlock = false;
    	for (int iter = 0; iter < 6; iter++ ) {
    		int tempX = i; int tempY = j; int tempZ = k;
    		if (iter == 0) {tempY--;} else if (iter == 1) {tempY++;}
    		else if (iter == 2) {tempZ--;} else if (iter == 3) {tempZ++;}
    		else if (iter == 4) {tempX--;} else if (iter == 5) {tempX++;}
    		if (world.getBlockId(tempX, tempY, tempZ) == mod_Elevator.ElevatorCaller.blockID
        ...
        mod_Elevator.say("ElevatorCaller activated at: " + i + ", " + j + ", " + k);
        ...
        mod_Elevator.say((new StringBuilder()).append("Found ").append(elvs.size()).append(" elevators").toString());
	        ...
	        if (newPos != null) {
	        	mod_Elevator.elevator_demandY(world, newPos, j);
	        	clearSets(depth);
	        	return true;
	        }
	        return false;
