...
public class GuiElevatorSlider extends GuiButton {
    ...
    private void nameString() {
        if (discrete) {
        	this.displayString = message + String.valueOf((int)this.sliderValue);
        }
        else {
    ...
    protected void mouseDragged(Minecraft par1Minecraft, int par2, int par3) {
        if (this.drawButton) {
            if (this.dragging) {
                ...
                if (discrete) { sliderPosition = MathHelper.floor_float( (sliderPosition*maximumValue + 0.5F))/maximumValue; }
