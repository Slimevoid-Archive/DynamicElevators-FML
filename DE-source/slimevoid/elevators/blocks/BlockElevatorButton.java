...
public class BlockElevatorButton extends BlockButton {
	...
	public BlockElevatorButton(int i, Icon icon, boolean sensible) {
		super(i, sensible);
		this.blockIcon = icon;
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int side, float par7, float par8, float par9) {
		...
		world.setBlockMetadataWithNotify(i, j, k, direction + state, 0x3);
	...
	public void updateTick(World world, int x, int y, int z, Random random) {
		if (!world.isRemote) {
			...
			if ((metadata & 8) != 0) {
				world.setBlockMetadataWithNotify(x, y, z, metadata & 7, 0x3);
				world.playSoundEffect(
						x + 0.5D,
						y + 0.5D,
						z + 0.5D,
