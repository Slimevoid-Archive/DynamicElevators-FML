...
public class GuiElevator extends GuiScreen {
		...
		for (int j = size; j > 0; j--) {
			...
			if (!DECore.invertKeys) {
				curButton = new GuiButton(j, startX + ((j - 1) % numCols)
												* spacing, startY
															- spacing
															* ((j - 1) / numCols), 20, 20, (new StringBuilder()).append(j).toString());
			} else {
				curButton = new GuiButton(j, startX - ((j - 1) % numCols)
												* spacing, startY
															- spacing
	...
	public void renameButtons() {
		for (int iter = 0; iter < floorButtons.size(); iter++) {
			...
			curButton.displayString = props.getAbbreviatedFloorName(curButton.id,
																	floorOne);
		}
		screenSubtitle = props.getExtendedFloorName(curFloor,
													floorOne);
	}
	...
	public void drawScreen(int i, int j, float f) {
		...
		if (!optionsOpen) {
			if (!screenTitle.equals("")) {
				drawUnshadedCenteredString(	fontRenderer,
											screenTitle,
											width / 2,
											titleTop,
											0x000000);
			...
			if (nameMode > 0) {
				...
				fontRenderer.drawString(request,
										width / 2 - 100,
										50,
										0x00000);
	...
	protected void actionPerformed(GuiButton guibutton) {
		...
		case GUI_OPTIONS_APPLY:
			...
			props.setBooleans(	canProvidePower.enabled,
								canBeHalted.enabled,
								mobilePower.enabled);
		...
		case GUI_OPTIONS_FLOORNAME:
			if (floorNamesList.selectedElement > -1) {
				...
				if (props.isFloorNamed(curSelectedFloor)) {
					txtEntryBox.setText(props.getExtendedFloorName(	curSelectedFloor,
																	floorOne));
				} else {
					txtEntryBox.setText("");
				}
	...
	protected void mouseClicked(int x, int y, int par3) {
		if (par3 == 0 && !floorNamesList.mousePressed(	x,
														y)
			&& floorNamesList.extended) {
			floorNamesList.minimize();
		} else if (floorNamesList.mousePressed(	x,
												y) && floorNamesList.extended) {
			return;
		}
	...
	protected void keyTyped(char par1, int par2) {
		this.txtEntryBox.textboxKeyTyped(	par1,
											par2);
	}
		...
		if (Keyboard.getEventKeyState()) {
			if (Keyboard.getEventKey() == GUI_CANCEL
				...
				floorNamesList.setAmountScrolled();
						...
						&& floorNamesList.selectedElement < (floorNamesList.getSize() - 1)) { // down
				floorNamesList.selectedElement++;
				floorNamesList.setAmountScrolled();
			}
		}
